...
def search():
    d = request.json
    max_len = min(int(d.get('max_len', 5)), 8)
    ...
    end_char = d.get('end_char', "").translate(VOICE_MAP if ignore_voiced else {})
    ...
    def solve(path):
        ...
        if len(path) == max_len:
            # 終了字チェック
            if end_char:
                tail = get_clean_char(path[-1], "tail").translate(VOICE_MAP if ignore_voiced else {})
