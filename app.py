...
def search():
    d = request.json
    max_len = min(int(d.get('max_len', 5)), 8) # 配布用は最大8単語までに制限
    ignore_voiced = d.get('ignore_voiced', False)
    must_char = d.get('must_char', "").translate(VOICE_MAP if ignore_voiced else {})
    limit_one = d.get('limit_one', False)

    results = []

    def solve(path):
        if len(results) >= 500: return # ヒット上限を絞る
        if len(path) == max_len:
            if must_char:
                full = "".join(path).translate(VOICE_MAP if ignore_voiced else {})
                if not all(c in full for c in must_char): return
                if limit_one and not all(full.count(c) == 1 for c in must_char): return
    ...
    starts = [start_word] if (start_word in COUNTRIES) else COUNTRIES
    for w in starts:
        if not start_word and d.get('start_char'):
