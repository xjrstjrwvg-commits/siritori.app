...
def get_clean_char(word, ignore_voiced, pos="head"):
    if not word: return ""
    c = word[0] if pos == "head" else word.replace("ー", "")[-1]
...
def get_target_char(word, shift_count, ignore_voiced, step, is_gyukou):
    ...
    # 接続元文字の選定
    base_char = word[0] if (is_gyukou and step % 2 == 0) else word.replace("ー", "")[-1]
    ...
    try:
        ...
        if ignore_voiced: res_char = res_char.translate(VOICE_MAP)
        return res_char
    except: return None
...
def search():
    ...
    raw_pool = DATA_STORE["countries"] if category == "countries" else DATA_STORE["capitals"] if category == "capitals" else DATA_STORE["countries"] + DATA_STORE["capitals"]
    ...
    if exclude_conjugate:
        ...
        new_pool = []
        for w in pool:
            h = get_clean_char(w, ignore_voiced, "head")
            t = get_clean_char(w, ignore_voiced, "tail")
            if h == t: continue # 自己共役排除 (アルジェリア等)
            if (h, t) in seen_pairs: continue # ペア重複排除 (アイスランドとアイルランド等)
            seen_pairs.add((h, t))
            new_pool.append(w)
        pool = new_pool
    ...
    def solve(path):
        ...
        if len(path) == max_len:
            if all(m in path for m in must_words): results.append(list(path))
            return
        ...
        for next_w in pool:
            if next_w not in path:
                next_comp = get_clean_char(next_w, ignore_voiced, "tail" if (is_gyukou and step % 2 != 0) else "head")
                if next_comp == target: solve(path + [next_w])

    start_word = d.get('start_word')
    start_candidates = [start_word] if start_word else pool
    for w in start_candidates:
        if w in pool: solve([w])

    # ソート
    sort_type = d.get('sort_type', 'default')
    if sort_type == 'length_desc': results.sort(key=lambda x: len("".join(x)), reverse=True)
    elif sort_type == 'shuffle': import random; random.shuffle(results)
