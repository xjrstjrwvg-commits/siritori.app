...
def search():
    ...
    base_depth = int(data.get('base_depth', 5))
    ...
    results = []
    min_d, max_d = base_depth - tolerance, base_depth + tolerance
    ...
    def backtrack(route, used):
        ...
        for word in pool:
            ...
            if mode == "gyoko":
                ...
                if mode == "normal": target = l_c
                elif mode == "shift": target = KANA_CYCLE[(idx+1)%len(KANA_CYCLE)] if idx!=-1 else l_c
                elif mode == "rev_shift": target = KANA_CYCLE[(idx-1)%len(KANA_CYCLE)] if idx!=-1 else l_c
